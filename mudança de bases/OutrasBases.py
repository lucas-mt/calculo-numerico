...
def trans_num(num, dicio, sig):
    vlr = 0
    for i in range(len(num)):
        if ver_int(num[i]):
           vlr += (int(num[i])*(16**(sig*i)))
        else:
...
def Dec_BinOct(num, base):
    parte_inteira = str(floor(num))
    parte_inteira_inversa = parte_inteira[::-1]
    parte_fracionada = str(num - int(parte_inteira)).replace('.', '')
    vlr = 0
    for i in range(len(parte_inteira_inversa)):
        vlr += (int(parte_inteira_inversa[i])*(base**i))
    for i in range(len(parte_fracionada)):
        vlr += (float(parte_fracionada[i])*(base**(-i)))
